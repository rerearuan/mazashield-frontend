...
export default function OrderDetailsModal({ isOpen, onClose, order, onSuccess }: OrderDetailsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await orderService.updateMazdagingOrder(order.id || order.id_pesanan, {
      ...
      toast.error(error.message || "Gagal memperbarui Pesanan Daging.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal size="lg" isOpen={isOpen} onClose={onClose} title={`Detail Pesanan Daging #${order.id_pesanan}`}>
        ...
        <div>
          ...
          <div className="space-y-2 border border-gray-100 rounded-xl overflow-hidden">
            {order.daftar_item?.map((item: any) => (
              <div key={item.id_daging} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 bg-white p-3 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                <div className="min-w-0">
                ...
                <p className="font-black text-sm text-gray-900 sm:text-right">Rp {parseFloat(item.subtotal_item).toLocaleString('id-ID')}</p>
              </div>
            ))}
          </div>
        </div>
